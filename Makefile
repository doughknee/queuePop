...
# ...or skip make entirely:  double-click scripts\run.cmd, or run  py src/main.py
#
...
help:
	...
	@echo   make build   - build the release .exe via scripts/build_release.py
	@echo   make clean   - remove build/ and __pycache__ artifacts

# Launch via scripts\run.cmd so the app gets its own (hidden) console and never
# hijacks the terminal you ran make from. `cmd /c` works whether make's
# shell is sh or cmd.
run:
	cmd /c .\scripts\run.cmd

# Kill the app via scripts\stop.cmd (kills the py/python process that launched
...
css:
	npx tailwindcss@3.4.17 -c scripts/tailwind.config.js -i src/webui/tailwind.src.css -o src/webui/styles.css --minify
